...
try:
    ...
    from quad_physics.envs.quadcopter_drone_env import QuadcopterDroneEnv
...
def render_rollout_saliency(env, frames, show=False):
    if render_saliency_suite is None:
        return
    path = [tuple(frame["state"]) for frame in frames]
    adapter = RolloutSaliencyAdapter(env.fixed_package, env.fixed_delivery, env.actions)
    try:
...
def visualize_trained_policy(model, src_env, delay=0.12, save_path=None, image_dir=None):
    ...
    render_rollout_saliency(env, frames, show=False)
